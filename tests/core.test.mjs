...
const { getPendingDividendSymbols } = await import('../src/render.js');
...
function applyTestSnapshot(overrides = {}) {
  ...
  if (!Object.prototype.hasOwnProperty.call(overrides, 'positionOpeningDate')
    && !Object.prototype.hasOwnProperty.call(overrides, 'openingDate')) snapshot.positionOpeningDate = '2020-01-01';
...
test('legacy auto yearly data migrates separately from manual overrides', () => {
  ...
  assert.equal(state.yearlyArchives[0].dividendCny, 0);
...
test('late official pay date patches automatic ledger and due remains in projected total', () => {
  applyTestSnapshot({
    ...
    dailySnapshots: [{
      date: '2026-06-01', netCny: 100, totalMarketValueCny: 100,
...
test('交易起点之前的股息不再按当前持仓倒推，以后年份仍可正常入账', () => {
  applyTestSnapshot({
  ...
  for (const [exDate, payDate, year] of [
    ...
    assert.ok(
      ...
      `${year} 年同期派息应按已知持仓正常入账`
    );
  }
  assert.deepEqual(state.dividendLedgerIgnored, [], '无需为系统生成的假历史记录建立墓碑');
});
...
/* 逐股行名称后的 4px 金点：UI 上造不出正向用例（事件只能由账本记录或行情机器人产出，
   真实数据常年只有 forecast），所以在这里把规则钉死，防止改渲染时把它悄悄改没。 */
test('holding gold dot lights up only for live dividend events', () => {
  const TODAY = '2026-07-10';

  // 在途（payDate 未到、未确认）→ 点亮
  applyTestSnapshot({
    dividendLedger: [{
      id: 'div_pending', sourceId: 'TEST.HK|2026-07-20|1|HKD', symbol: 'TEST.HK',
      exDate: '2026-07-20', payDate: '2026-08-01', amountPerShare: 1, currency: 'HKD',
      shares: 10, fxRate: 1, taxRate: 0, grossCny: 10, netCny: 10,
      bucket: 'core', receiptStatus: 'pending', confidence: 'manual', confirmed: false
    }]
  });
  assert.equal(computeDividendCalendar(TODAY).allDetails.find((e) => e.id === 'div_pending').status, 'pending');
  assert.ok(getPendingDividendSymbols().has('TEST.HK'), '在途股息应点亮金点');

  // 待核对（payDate 已过但没勾确认）→ 也点亮
  state.dividendLedger[0].payDate = '2026-07-08';
  invalidateComputeCache();
  assert.equal(computeDividendCalendar(TODAY).allDetails.find((e) => e.id === 'div_pending').status, 'due');
  assert.ok(getPendingDividendSymbols().has('TEST.HK'), '待核对股息应点亮金点');

  // 已确认到账 → 熄灭
  state.dividendLedger[0].confirmed = true;
  state.dividendLedger[0].receivedDate = '2026-07-08';
  invalidateComputeCache();
  assert.equal(computeDividendCalendar(TODAY).allDetails.find((e) => e.id === 'div_pending').status, 'received');
  assert.ok(!getPendingDividendSymbols().has('TEST.HK'), '已到账不该再点亮');

  // 已公告未除息（行情机器人写进 quotes 的主路径）→ 点亮
  applyTestSnapshot({
    quotes: {
      'TEST.HK': {
        name: 'Test', price: 10, currency: 'HKD',
        dividends: [{ exDate: '2026-08-15', payDate: '2026-09-01', amountPerShare: 1, currency: 'HKD', status: 'announced' }]
      }
    }
  });
  assert.ok(
    computeDividendCalendar(TODAY).allDetails.some((e) => e.symbol === 'TEST.HK' && e.status === 'announced'),
    '公告事件应进日历'
  );
  assert.ok(getPendingDividendSymbols().has('TEST.HK'), '已公告未除息应点亮金点');
  ...
  applyTestSnapshot({
    quotes: {
      'TEST.HK': {
        name: 'Test', price: 10, currency: 'HKD',
        dividends: [{ exDate: '2025-09-15', payDate: '2025-10-01', amountPerShare: 1, currency: 'HKD' }]
      }
    }
  });
  const forecastOnly = computeDividendCalendar(TODAY).allDetails.filter((e) => e.symbol === 'TEST.HK');
