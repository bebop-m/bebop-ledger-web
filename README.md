...
已除息事件取除息日前最近一份日快照，并回放快照之后、除息日之前的交易，以此冻结股数、税率和汇率；除息日交易不改变该次权利。缺少历史锚点时不会用当前持仓倒推过去，而是明确标记为未验证并排除。跨来源的聚合记录与分项记录只有在金额可回勾时才折叠；同来源的多次合法派息保持独立。

“预计全年同比”以去年账本中按实际到账日或有效派付日归年的已确认股息为基准，与收益页的年度实际股息保持同一口径。
...
当前现金是一次直接快照，保存时不会回放历史交易、出入金或股息，也不会改变持股数量。快照日之后新发生的交易、出入金和已确认到账股息会继续自动更新现金；现金可以为负，用于表示融资或透支。
...
结算会按日期 upsert 日快照；同一天的持仓、现金、负债或汇率变化会覆盖当天旧值，确保跨端多次结算仍只有一条当日事实。股息权利按除息日前一日的最近快照冻结，并回放快照后、除息日前的交易；缺少历史锚点时不会用当前持仓倒推过去。
...
2. 读取私有快照，按业务主键、更新时间和删除墓碑与本地账本合并。
3. 将新增证券代码追加到公开 `data/watchlist.json`。
4. 刷新行情并完成结算后，以读取时的 SHA 条件写入私有仓；并发修改会报冲突而非静默覆盖。
5. 必要时触发市场数据任务；新行情部署后再次合并并上传最终结算结果。
...
`npm test` 会运行前端公式、边界案例以及私有结算脚本的跨端一致性测试。生产构建用 `npm run build`。
